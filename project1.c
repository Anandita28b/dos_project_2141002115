...
if (remaining_burst_time[i] >= time_quantum) { 
  current_time += time_quantum; remaining_burst_time[i] -= time_quantum;
   printf("Executing process %d from time %d to %d\n", processes[i].process_id,
current_time - time_quantum, current_time);
} else { current_time += remaining_burst_time[i]; 
...
int main(){ struct Process processes[] = {{1, 0, 10},{2, 1, 1},{3, 2, 2},{2, 3, 1},{3, 6, 5}};
